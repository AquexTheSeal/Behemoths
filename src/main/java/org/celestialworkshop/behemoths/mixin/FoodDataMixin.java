...
public abstract class FoodDataMixin {
    ...
    private float foodLvlRedThresh(float value, Player player) {
        if (PandemoniumVotingSystem.hasPandemoniumCurse(player.level(), BMPandemoniumCurses.FRAGILITY)) {
            return value * 0.5F;
        }
        return value;
    }
    ...
    @ModifyConstant(method = "tick", constant = @Constant(floatValue = 1.0F, ordinal = 1), require = 0)
